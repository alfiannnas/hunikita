...
export class Repository implements IRepository {
    ...
    async updateStatusSewa(uuid: string, status_sewa: string): Promise<RowDataPacket> {
        try {
            const [result] = await this.master.execute(
                `UPDATE penyewa SET status = ?, updated_at = NOW() WHERE uuid = ?`,
                [status_sewa, uuid]
            )
            return result as RowDataPacket
        } catch(error) {
